...
function top2(){
   	form3.action="<%=basePath%>admin/chu/index.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/chu/index.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/chu/index.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/chu/index.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/chu/index.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/chu/index.jsp?page="+page;
      form3.submit();
...
<div class="title_right" style="background:#e5f9fc;"><strong>出院管理</strong></div>  
...
<%   
	...
	int cou = cb.getMessageCount("select count(*) from ap  where zt='已出院'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from ap where zt='已出院' order by id desc",13);
